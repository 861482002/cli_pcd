...
def bindPlane(src_files:str, target_file:str,args):
	...
	tar_planarity = np.array([tar_planarity])[None, :] * 10
	...
	for pcd, center in zip(postreg_source, centers_ply_lst):
		...
		# 平面度
		plane = fit_plane(pcd)
		planarity_ply_lst.append((calculate_planarity(pcd, plane) * 10))
	...
	for matched_idx in matched1_idxs:
		...
		# o3d.visualization.draw_geometries([selected_cad_plane_show, match_cad_show,axis])
	...
	# 返回包含中文的json
	json_res = json.dumps(res_dict, ensure_ascii=False)
...
if __name__ == '__main__':
	# 初始化
	# src_files = '#'.join([ os.path.join('./零件1数据/实测',file) for file in os.listdir('./零件1数据/实测') if len(file) == 7])
	# tar_files = [ os.path.join('./零件1数据/目标',file) for file in os.listdir('./零件1数据/目标') if len(file) == 7]
	# for tar_file in tar_files:
	# 	print(tar_file)
	# 	bind_plane = bindPlane(src_files, tar_file, None)
	# 	print(bind_plane)
	# 	print('开始下一个平面\n')
	...
	# tar_file = './零件1数据/目标/PG6.txt'
	# tar_file = './零件2数据/目标/PG6.txt'
	# tar_file = './零件3数据/目标/PG2.txt'

	# src_files = '#'.join([ os.path.join('./零件3数据/实测',file) for file in os.listdir('./零件3数据/实测') if len(file) == 7])
	# tar_file = './零件3数据/目标/PG2.txt'
	...
	# --src ./零件1数据/实测\PG0.txt#./零件1数据/实测\PG1.txt#./零件1数据/实测\PG2.txt#./零件1数据/实测\PG3.txt#./零件1数据/实测\PG4.txt#./零件1数据/实测\PG5.txt --tar ./零件1数据/目标\PG2.txt

	# --src ./零件2数据/实测\PG0.txt#./零件2数据/实测\PG1.txt#./零件2数据/实测\PG2.txt#./零件2数据/实测\PG3.txt --tar ./零件2数据/目标\PG2.txt

	# --src ./零件3数据/实测\PG0.txt#./零件3数据/实测\PG1.txt#./零件3数据/实测\PG2.txt#./零件3数据/实测\PG3.txt#./零件3数据/实测\PG4.txt#./零件3数据/实测\PG5.txt --tar ./零件3数据/目标\PG4.txt
	pass
