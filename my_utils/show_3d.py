...
def show_3d_file(file_name: str,arg_color=None):
...
def show_3d_files(file_names: List[str], arg_color=None):

	show_list = []
	for file,color in zip(file_names, arg_color):
...
def show_3d_all(file_names: List[str]):
...
def visualize_point_clouds(pcd1:np.ndarray, pcd2:np.ndarray, data1, data2):
	...
	ax1.legend()
	ax1.text2D(0.05, 0.95, f'平面度：{data1}', transform=ax1.transAxes, fontdict=fontdict)
	ax1.text2D(0.05, 1, f'曲面度：{data2}', transform=ax1.transAxes, fontdict=fontdict)
	# 第二个子图
