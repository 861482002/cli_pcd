...
def segmentation_plan_file(file_name:str , out_dir:str = None):
    ...
    if not os.path.exists(out_dir):
        os.makedirs(out_dir)
    o3d.io.write_point_cloud(os.path.join(out_dir, f'{0:03d}f.ply'), inlier_cloud,write_ascii=True)
    ...
    while res_points_num > 10000:
        ...
        # 保存接下来分割出来的面
        if len(inliers) > 5000:
            o3d.io.write_point_cloud(os.path.join(out_dir,  f'{save_idx:03d}f.ply'), inlier_cloud,write_ascii=True)
            save_idx += 1
        res_points_num = np.asarray(outlier_cloud.points).shape[0]
        # 可视化分割结果
...
if __name__ == '__main__':
    # file_name = '../data/1.ply'
    # out_dir = '../data/segmentaion_ply'
    # segmentation_plan_file(file_name,out_dir)
